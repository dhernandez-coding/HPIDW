CREATE view [fact].[vTestingTransactionsPB] as


-- this is so funny RR 8-20
/* Look to replace complicated join structure with CTEs to improve performance - 5/10/24 */
--WITH ChargePayerID_CTE AS (
--    SELECt TransactionSourceID, TransactionParentSourceID, TransactionPayerID, TransactionGLType, TransactionDepartmentID, TransactionCPTCode
--    FROM fact.Transactions2
--    WHERE TransactionType = 'charge' and TransactionSubType <> 'charge - void' and TransactionBillingType = 'PB'
--)

WITH ChargePayerID_CTE AS (
    SELECt TransactionSourceID, TransactionParentSourceID, TransactionPayerID, TransactionGLType, TransactionDepartmentID, TransactionCPTCode, TransactionCode, TransactionType, TransactionDatasourceID
    FROM fact.Transactions2
    WHERE TransactionType = 'charge' and TransactionSubType <> 'charge - void' and TransactionBillingType = 'PB'
	)
...
FROM (
	...
	*/						  
	...
	LEFT JOIN ChargePayerID_CTE tc ON tc.TransactionSourceID = t.TransactionSourceID
	left join stg.PBProcedureCodeCategories cc ON cc.ProcedureCode = tc.TransactionCode AND tc.TransactionType = 'Charge'
	left join dim.vProcedureCodes pcpt on pcpt.ProcedureCode = t.TransactionCPTCode and pcpt.ProcedureCodeDataSourceID = t.TransactionDatasourceID
	left join dim.vProcedureCodes pcpt2 on pcpt2.ProcedureCode = tc.TransactionCPTCode and pcpt2.ProcedureCodeDataSourceID = tc.TransactionDatasourceID
