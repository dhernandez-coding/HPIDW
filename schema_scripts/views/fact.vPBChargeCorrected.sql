--I edited the MidLevel to a more loosy match condition so if is not on this list ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') we are not pulling the midlevel values
/***
7/22/2026 - Logan RIchardson - Replaced direct hero db reference to PBProcedureCategories to dim.vPBProcedureCategories

***/



CREATE VIEW [fact].[vPBChargeCorrected] as
...
FROM (
	...
	SELECT
		...
		,CASE WHEN bc.CorrectPlan ='BlueChoice' then '5~19007'
			  WHEN bc.CorrectPlan ='BluePreferred' then '5~19007'
			  WHEN bc.CorrectPlan ='BlueAdvantage' then '5~19007'
			  WHEN bc.CorrectPlan ='BlueTraditional' then '5~19007'
			  ELSE t.[TransactionPayerID] END as [TransactionPayerID]
		,CASE WHEN bc.CorrectPlan ='BlueChoice' then '5~1900702'
			  WHEN bc.CorrectPlan ='BluePreferred' then '5~1900705'
			  WHEN bc.CorrectPlan ='BlueAdvantage' then '5~1900701'
			  WHEN bc.CorrectPlan ='BlueTraditional' then '5~1900706'
			  ELSE t.[TransactionPayerPlanID] END as [TransactionPayerPlanID]
		...
		,CASE
			    WHEN t.TransactionPlaceOfServiceCode IN ('21','22') 
			         AND pp.PracticeProviderGLType IN ('PA','APRN') AND mfs.FeeScheduleID IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') THEN mfs.FeeScheduleFacilityRateMidLevel
			    WHEN t.TransactionPlaceOfServiceCode IN ('21','22') THEN mfs.FeeScheduleFacilityRate
			    WHEN t.TransactionPlaceOfServiceCode NOT IN ('21','22') 
			         AND pp.PracticeProviderGLType IN ('PA','APRN') AND mfs.FeeScheduleID IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred')  THEN mfs.FeeScheduleNonFacilityRateMidLevel
			    ELSE mfs.FeeScheduleNonFacilityRate 
			END AS UnmodifiedFeeSchedule
		,(CASE
			    WHEN t.TransactionPlaceOfServiceCode IN ('21','22') 
			         AND pp.PracticeProviderGLType IN ('PA','APRN') AND mfs.FeeScheduleID IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') THEN mfs.FeeScheduleFacilityRateMidLevel
			    WHEN t.TransactionPlaceOfServiceCode IN ('21','22') THEN mfs.FeeScheduleFacilityRate
			    WHEN t.TransactionPlaceOfServiceCode NOT IN ('21','22') 
			         AND pp.PracticeProviderGLType IN ('PA','APRN')  AND mfs.FeeScheduleID IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') THEN mfs.FeeScheduleNonFacilityRateMidLevel
			    ELSE mfs.FeeScheduleNonFacilityRate 
			END  * COALESCE(RVU.Adjustment,RVU2.Adjustment,RVU3.Adjustment,RVU4.Adjustment)) as ModifiedFeeSchedule

		,CASE WHEN pp.PracticeProviderGLType in ('PA','APRN') AND mfs.FeeScheduleID NOT IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') THEN .85 ELSE 1 END as MidlevelDiscount
		,CASE 
			WHEN bc.CorrectPlan IS NULL --OR bc.CorrectPlan = '0~BlueAverage'
			THEN 
			    CASE 
			        WHEN YEAR(t.TransactionDateOfService) >= 2024 
			        THEN 
			        (
			            (CASE 
			                WHEN Mult.row_num = 1 THEN 0.5                                                                                --MultProcedure
			                WHEN Mult.row_num > 1 THEN 0.25
			                ELSE 1 
			            END) 
			            * 
			            (CASE 
			                WHEN ISNULL(COALESCE(RVU.Adjustment, RVU2.Adjustment, RVU3.Adjustment, RVU4.Adjustment), 0) = 0 
			                THEN 
			                ...
			                ELSE 
			                    CASE
			                        ...
			                        ELSE mfs.FeeScheduleNonFacilityRate
			                    END
			                    * COALESCE(RVU.Adjustment, RVU2.Adjustment, RVU3.Adjustment, RVU4.Adjustment)
			            END) 
			            * 
			            
			            CASE 
			                WHEN pp.PracticeProviderGLType IN ('PA','APRN') AND mfs.FeeScheduleID NOT IN ('0~BlueCrossTraditional', '0~BlueAverage', '0~BlueCrossAdvantage','0~BlueCrossChoice', '0~BlueCrossPreferred') THEN 0.85                                                                                   -- Modifier
			                ELSE 1 
			            END 
			            * 
			        ...
			        * t.[TransactionUnits]
			        ELSE NULL
			    END 
			ELSE bc.SumAllowable 
		END	AS CalculatedFeeSchedule
		,CASE WHEN bc.CorrectPlan IS NOT NULL AND bc.CorrectPlan <> 'Other problem' THEN bc.CorrectPlan ELSE fsc.FeeScheduleName END as FeeScheduleName
		,CASE WHEN bc.CorrectPlan ='BlueChoice' then '0~BlueCrossChoice'
			  ...
			  ELSE fs.FeeScheduleID END as FeeScheduleID
		...
		--NEED To chan ge the payer, and payer plan here 
		...
		,--CASE WHEN t.TransactionType = 'Payment' THEN
			CASE WHEN t.TransactionGLType = 'RadTC' THEN '41' /*Rad TC*/
				 WHEN t.TransactionDepartmentID = '1~10' THEN '61' /*PI*/
	...
	FROM fact.TransactionsPB t
		LEFT join rpt.BCBSCorrected bc on bc.TransactionSourceID = CONCAT(t.[TransactionDatasourceID],'~',t.[TransactionParentSourceID]) and t.TransactionType ='Charge'
		...
		LEFT JOIN dim.FeeSchedules fsc ON fsc.FeeScheduleID = COALESCE(CASE WHEN bc.CorrectPlan ='BlueChoice' then '0~BlueCrossChoice'
																				  WHEN bc.CorrectPlan ='BluePreferred' then '0~BlueCrossPreferred'
																				  WHEN bc.CorrectPlan ='BlueAdvantage' then '0~BlueCrossAdvantage'
																				  WHEN bc.CorrectPlan ='BlueTraditional' then '0~BlueCrossTraditional'
																				  ELSE fs.FeeScheduleID END
																			,fs.FeeScheduleID)
		LEFT JOIN dim.FeeScheduleRates mfs ON mfs.FeeScheduleID = COALESCE(CASE WHEN bc.CorrectPlan = 'BlueChoice' THEN '0~BlueCrossChoice'
											    WHEN bc.CorrectPlan = 'BluePreferred' THEN '0~BlueCrossPreferred'
											    WHEN bc.CorrectPlan = 'BlueAdvantage' THEN '0~BlueCrossAdvantage'
											    WHEN bc.CorrectPlan = 'BlueTraditional' THEN '0~BlueCrossTraditional' -- simplify this join
...
--# PREVIOUS VERSION NON FEESCHEDULE LOADED MIDLEVEL
--SELECT 
--	sub.*
--	, CASE WHEN sub.TransactionGLSegmentType = '99' THEN '00' /*Administrative*/ 
--			 WHEN sub.TransactionPracticeID = '0~TDT' AND sub.TransactionVisitLocationID in ('1~95','1~135') THEN '22' /*Thomason - Hinton*/ 
...
--                            CASE 
--                                WHEN t.TransactionPlaceOfServiceCode IN ('21','22') 
--                                THEN mfs.FeeScheduleFacilityRate 
--                                ELSE mfs.FeeScheduleNonFacilityRate 
--                            END  
--                        ELSE 
--                            CASE 
...
--                    *
--                    CASE 
--                        WHEN pp.PracticeProviderGLType IN ('PA','APRN') 
--                        THEN .85 --If i dont include this here the variance increase
--                        ELSE 1 
--                    END 
--                    * 
--                    ISNULL(fsc.FeeScheduleRateMultiplier, 1)
...
--				ELSE bc.SumAllowable 
--			END AS CalculatedFeeSchedule
--		,CASE WHEN bc.CorrectPlan IS NOT NULL THEN bc.CorrectPlan ELSE fsc.FeeScheduleName END as FeeScheduleName
--		,CASE WHEN bc.CorrectPlan ='BlueChoice' then '0~BlueCrossChoice'
--			  WHEN bc.CorrectPlan ='BluePreferred' then '0~BlueCrossPreferred'
--			  WHEN bc.CorrectPlan ='BlueAdvantage' then '0~BlueCrossAdvantage'
--			  WHEN bc.CorrectPlan ='BlueTraditional' then '0~BlueCrossTraditional'
...
--		LEFT JOIN dim.Patients p ON p.PatientID = a.AccountPatientID -- for patient MRN 2/1/24
--		left join dim.vPBProcedureCodeCategories c ON c.ProcedureCode = COALESCE(t.TransactionCPTCode,t.TransactionCode) AND t.TransactionType = 'Charge'
--		left join HPIApp.dbo.PBProcedureCategories cat ON cat.ProcedureCategory = CASE WHEN c.ProcedureCodeIsLocationDependent = 1 and t.TransactionPlaceOfServiceCode in ('21','22') THEN 'Outpatient Procedures' 
--																				WHEN c.ProcedureCodeIsLocationDependent = 1 and t.TransactionPlaceOfServiceCode not in ('21','22') THEN 'In Office Procedures'
--																				ELSE c.ProcedureCodeCategory END
--		left join dim.DataSources ds ON t.TransactionDatasourceID = ds.DataSourceID
--		left join map.vProviderLinking pl ON pl.ChildProviderID = t.TransactionBillingProviderID
