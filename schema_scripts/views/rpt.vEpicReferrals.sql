--EXEC stg.spHPIReloadDimPracticesFull
--EXEC stg.spHPIReloadMapPracticeProvidersFull

----select * from rpt.vEpicReferrals where ReferralID in (10026746)

--select * from HPIApp.dbo.PracticeProviders p where p.ProviderID in (159469,156062)

--select * from HPIApp.dbo.providers p where p.ProviderLastName = 'Biggers'



CREATE VIEW [rpt].[vEpicReferrals] as

/*11/22/24 - Chris Cross - Changed to reference new PracticeProviders mapping from HPI App*/
	...
	select r.[REFERRAL_ID] as ReferralID
		   ...
		   ,case when rf.REFERRAL_ID is null THEN 'No' ELSE 'Yes' END as IsWorkersComp
		   --,rh.HX_USER_ID
	...
	from [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].REFERRAL r
		...
		LEFT JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].REFERRAL_FLAGS rf ON rf.REFERRAL_ID = r.REFERRAL_ID AND rf.FLAG_C = 117040
		--LEFT JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].ZC_RFL_FLAG fl ON fl.RFL_FLAG_C = rf.FLAG_C 
		...
		left join map.PracticeLocations lt ON lt.LocationID = CONCAT('5~',r.[REFD_TO_LOC_POS_ID])
		...
		left join dim.zTEST_Practices prt ON prt.PracticeID = CASE --WHEN CONCAT('5~',c1.PROV_ID) = '5~125582' AND dt.PracticeID IS NOT NULL THEN dt.PracticeID /*Exception for Murphi Scarborough who practices at 2 clinics*/
															 ...
															 WHEN dt.PracticeIDNew IS NOT NULL THEN dt.PracticeIDNew
