...
CREATE VIEW [rpt].[vEpicReferrals] as
	...
	select r.[REFERRAL_ID] as ReferralID
		   ...
		   ,ppb.IsAffiliate as IsReferredByAffiliate
		   ,ppb.IsHPISpecialist as IsReferredByHPISpecialist
		   ,ppb.IsHPIPrimaryCare as IsReferredByHPIPrimaryCare
		   ,ppb.IsReferralTarget as IsReferredByTarget
		   ...
		   ,ppt.IsAffiliate as IsReferredToAffiliate
		   ,ppt.IsHPISpecialist as IsReferredToHPISpecialist
		   ,ppt.IsHPIPrimaryCare as IsReferredToHPIPrimaryCare
		   ,ppt.IsReferralTarget as IsReferredToTarget
	...
	from [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].REFERRAL r
		...
		left join dim.Practices prb ON prb.PracticeID = CASE --WHEN CONCAT('5~',c.PROV_ID) = '5~125582' AND db.PracticeID IS NOT NULL THEN db.PracticeID /*Exception for Murphi Scarborough who practices at 2 clinics*/
															 ...
															 WHEN r.[REFERRING_PROV_ID] is not null THEN '0~EXT0' /*Map all unmapped records to External as long as something exists in the Referred By*/
															 END
		left join dim.Practices prt ON prt.PracticeID = CASE --WHEN CONCAT('5~',c1.PROV_ID) = '5~125582' AND dt.PracticeID IS NOT NULL THEN dt.PracticeID /*Exception for Murphi Scarborough who practices at 2 clinics*/
															 ...
															 ELSE '0~EXT0' /*Map all unmapped records to External as long as something exists in the Referred To*/
														 END--COALESCE(ppt.PracticeID,dt.PracticeID)

	where 1=1 --and r.REFERRAL_ID IN (2183315,3037728,2136956)
