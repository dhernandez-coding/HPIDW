...
-- Edit date:   1. 8/12/26  - Diego Hernandez - Moving to openquery 
...
/*-----INSERT INTO @StagingTable-----*/
    ...
    SELECT 
        CONCAT('5~', r.TX_ID) AS SelfPayTransactionID
        ,5 AS SelfPayDataSourceID
        ,r.TX_ID AS SelfPayTransactionSourceID
        ,CONCAT('5~', r.PAT_ENC_CSN_ID) AS SelfPayVisitID
        ,CONCAT('5~', r.PAT_ID) AS SelfPayPatientID
        ,r.TX_TYPE AS SelfPayType
        ,r.SERVICE_DATE AS SelfPayDateOfService
        ,r.POST_DATE AS SelfPayPostDate
        ,r.POST_DATE_AGE AS SelfPayPostDateAge
        ,r.POST_DATE_AGE_BKT AS SelfPayPostDateAgeBucket
        ,r.SELF_PAY_AGE_DATE AS SelfPayAgeDate
        ,r.SELF_PAY_AGE AS SelfPayAge
        ,r.SELF_PAY_AGE_BKT AS SelfPayAgeBucket
        ,r.CURRENT_FINANCIAL_CLASS_NAME AS SelfPayFinancialClass
        ,r.OUTSTANDING_SELF_PAY_AMOUNT AS SelfPayBalance
        ,r.PATIENT_AMT AS SelfPayPatientResponsibility
        ,r.OUTSTANDING_AMT AS SelfPayOutstandingBalance
    ...
    FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM], '
        SELECT
            ...
            sp.SERVICE_DATE,
            sp.POST_DATE,
            MAX(sp.POST_DATE_AGE) AS POST_DATE_AGE,
            MAX(sp.POST_DATE_AGE_BKT) AS POST_DATE_AGE_BKT,
            sp.SELF_PAY_AGE_DATE,
            MAX(sp.SELF_PAY_AGE) AS SELF_PAY_AGE,
            MAX(sp.SELF_PAY_AGE_BKT) AS SELF_PAY_AGE_BKT,
            sp.CURRENT_FINANCIAL_CLASS_NAME,
            sp.OUTSTANDING_SELF_PAY_AMOUNT,
            t.PATIENT_AMT,
            t.OUTSTANDING_AMT
        FROM CLARITY.ORGFILTER.V_ARPB_ATB_TX_DETAIL sp
        LEFT JOIN CLARITY.ORGFILTER.PAT_ENC p ON p.PAT_ENC_CSN_ID = sp.PAT_ENC_CSN_ID
        LEFT JOIN CLARITY.ORGFILTER.ARPB_TRANSACTIONS t on t.TX_ID = sp.TX_ID
