...
-- Description:	Extracts, Transforms and Loads Patient Data from Medhost CH Source System into a dim Table
-- Change Control
--	1. 03/10/2023 - Chris Cross - Initial build of procedure
-- =============================================
CREATE PROCEDURE [stg].[spMedhostCHReloadDimPatientsFull]
AS
BEGIN
SET NOCOUNT ON;

delete from dim.Patients where PatientDataSourceID = 8
...
select
	'8~' + CONVERT(varchar(100),p.HISTN) as PatientID,
	8 as PatientDataSourceID,
...
FROM OPENQUERY([hmsls],'
	select
	*
	from HOSPF110.PATHISTM --DICTIONARY
	where 1=1 
	') p
END
GO
