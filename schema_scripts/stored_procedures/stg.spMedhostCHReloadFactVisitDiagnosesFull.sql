CREATE PROCEDURE [stg].[spMedhostCHReloadFactVisitDiagnosesFull] as
  
  DELETE FROM fact.VisitDiagnoses WHERE VisitDiagnosisDataSourceID = 8

  INSERT INTO fact.VisitDiagnoses
  SELECT 
	CONCAT('8~',v.DXNUMB,'~',v.DXTYPE,'~',v.DXSEQ#,'~',v.DXCMED,'~',v.DXICD9) AS VisitDiagnosisID
	,8 as VisitDiagnosisDataSourceID
	,CONCAT(v.DXNUMB,'~',v.DXTYPE,'~',v.DXSEQ#,'~',v.DXCMED,'~',v.DXICD9) as VisitDiagnosisSourceID
	,CONCAT('8~',v.DXNUMB) as VisitDiagnosisVisitID
	,CONCAT('8~',v.DXNUMB) as VisitDiagnosisAccountID
  ...
  FROM OPENQUERY([hmsls],'
	...
	,dx.DXCDDT
	--dx.*
	from MHD32.HOSPF110.MRABDIAG dx
		LEFT JOIN MHD32.HOSPF110.I10DIAG icd ON icd.DGCODE_ = dx.DXICD9 
