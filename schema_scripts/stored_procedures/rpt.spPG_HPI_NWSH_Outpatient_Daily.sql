...
-- =============================================
*/
CREATE PROCEDURE [rpt].[spPG_HPI_NWSH_Outpatient_Daily] @STARTDATE date, @ENDDATE date
AS
...
---        Service Area = 430
---        Location = NPI NORTHWEST SURGICAL HOSPITAL
...
set @filename
     = 'HPI_NWSH_OP_Daily_' + CONVERT(NVARCHAR(10), @STARTDATE, 112) + '_to_' + CONVERT(NVARCHAR(10), @ENDDATE, 112)

IF OBJECT_ID(N'tempdb..##TempNWOP') IS NOT NULL
DROP TABLE ##TempNWOP

IF EXISTS (SELECT * FROM tempdb.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '#TempAccounts%') 
BEGIN 
	Print 'DROP TABLE #TempAccounts'
	DROP TABLE #TempAccounts
END;

IF EXISTS (SELECT * FROM tempdb.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '#cteProviders%') 
BEGIN 
	Print 'DROP TABLE #cteProviders'
	DROP TABLE #cteProviders
END;

--IF EXISTS (SELECT * FROM tempdb.INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '#cteConfidentialPatients%') 
--BEGIN 
--	Print 'DROP TABLE #cteConfidentialPatients'
...
/*INSERT Filtered Records into #TempAccounts*/
	...
	where 1=1
		...
		and eh.adt_pat_class_c NOT IN (101, 132, 167, 107, 108) /*Inpatient, Rehab Inpatient, Home Inpatient, Newborn, Surgery Admit*/
...
select
	'OU0101' PressGaneySurgeryDesignator -- ER0101 reporttype/file hardcode
		,case
	...
	,CASE 
		WHEN a.acct_basecls_ha_c = 2 THEN 'Outpatient' END AS PatientClass
	,REPLACE(CONVERT(VARCHAR(10), a.adm_date_time, 101), '/', '') ApptDate
	,'' ApptCheckoutTime --changed to blank to reflect extract
	,ISNULL(a.PATIENT_STATUS_C, '') DischargeDisposition --place or setting? 
...
from #TempAccounts sub 
	INNER JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].HSP_ACCOUNT a ON a.HSP_ACCOUNT_ID = sub.HSP_ACCOUNT_ID
	INNER JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].PAT_ENC_HSP eh ON eh.PAT_ENC_CSN_ID = a.PRIM_ENC_CSN_ID
	LEFT JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].PAT_ENC e ON e.PAT_ENC_CSN_ID = eh.PAT_ENC_CSN_ID
	LEFT JOIN [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].pat_enc_hsp_2 e2 on e.pat_enc_csn_id = e2.pat_enc_csn_id
...
WHERE 1=1
	AND sub.ADT_PATIENT_STAT_C in (3,6) /*Discharged, HOV*/
	AND (sub.APPT_STATUS_C IS NULL or sub.APPT_STATUS_C in (2,6)) /*Arrived, Completed*/
	AND LEFT(sub.hsp_account_id,1) = '6' /*Only want Hospital*/
	AND sub.Survey_Opt_Out_YN  <> 'Y' /*Survey Opt Out <> Yes*/
	AND sub.department_id LIKE '43006001%'
...
'CAST(EORIndicator as varchar(1)) FROM ##TempNWOP'
