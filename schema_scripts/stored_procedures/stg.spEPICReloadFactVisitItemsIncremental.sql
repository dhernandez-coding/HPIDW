...
-- Author:		
-- Create date: 
-- Description:	Extracts, Transforms and Loads VisitItems Data from EPIC Source System into a fact table
-- Change Control
...
DELETE FROM fact.VisitItems WHERE VisitItemDataSourceID = 5 AND VisitItemUsedDate >= DATEADD(DAY,-@DaysToReload, convert(date,GETDATE())) /*Only load transactions from the last 10 days*/
    ...
    SELECT 
        ...
        ,CONCAT('5~', r.OR_LINK_CSN) as VisitID
        ,CONCAT('5~', r.OR_CASELOG_ID) as VisitCaseID
        ,CONCAT('5~', COALESCE(r.ITEM_ID, r.IMPLANT_ID)) as ItemID
        ,r.CHARGE_CODE as ItemChargeCode
        ,NULL AS ItemRevenueCode
        ,NULL AS ItemRevenueCodeDescription
        ,COALESCE(r.ITEM_NM, r.IMPLANT_NM) AS ItemDescription
        ,CASE 
            WHEN r.ITEM_ID in (163333,163068,169485) THEN 'Monitoring'
            WHEN r.ITEM_ID in (151738,156376) THEN 'Cell Savers'
            WHEN r.ITEM_TYPE = 'Supply' THEN 'Supplies'
            WHEN r.ITEM_TYPE = 'Implant' THEN 'Implants'
            ELSE r.ITEM_TYPE 
         END as ItemType
        ,r.TYPE_NM as ItemSubtype
        ,ISNULL(r.NUMBER_USED,0) + ISNULL(r.NUMBER_WASTED,0) AS VisitItemQuantity
        ,ISNULL(r.COST_PER_UNIT,0) AS VisitItemUnitCost
        ,ISNULL(
            CASE 
                WHEN r.ITEM_TYPE = 'Supply' THEN (ISNULL(r.NUMBER_USED,0) + ISNULL(r.NUMBER_WASTED,0)) * ISNULL(r.COST_PER_UNIT,0) 
                WHEN r.ITEM_TYPE = 'Implant' AND r.CHARGEABLE_YN = 'Y' THEN (ISNULL(r.NUMBER_USED,0) + ISNULL(r.NUMBER_WASTED,0)) * ISNULL(r.COST_PER_UNIT,0) 
            END, 0
         ) as VisitItemExtendedCost
        ,CONVERT(DATE, r.EFFECTIVE_DATE_DTTM) as VisitItemUsedDate
        ,CASE WHEN r.CHARGEABLE_YN = 'Y' THEN 1 ELSE 0 END as VisitItemIsChargeable
    ...
    FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM], '
       DECLARE @DaysToReload int = 10 
        ...
        FROM CLARITY.ORGFILTER.v_log_supplies_implants si 
        LEFT JOIN CLARITY.ORGFILTER.PAT_OR_ADM_LINK orl ON orl.OR_CASELOG_ID = si.CASE_ID
        LEFT JOIN CLARITY.ORGFILTER.PAT_ENC enc ON enc.PAT_ENC_CSN_ID = orl.PAT_ENC_CSN_ID
        LEFT JOIN CLARITY.ORGFILTER.PAT_ENC_HSP hsp ON hsp.PAT_ENC_CSN_ID = orl.OR_LINK_CSN 
        LEFT JOIN CLARITY.ORGFILTER.OR_SPLY s ON s.SUPPLY_ID = si.ITEM_ID AND si.ITEM_TYPE = ''Supply''
        LEFT JOIN CLARITY.ORGFILTER.OR_IMP i ON i.IMPLANT_ID = si.IMPLANT_ID AND si.ITEM_TYPE = ''Implant''
    ...
    SELECT 
        5 as VisitItemDataSourceID
        ,CONCAT('5~', r.HSP_ACCOUNT_ID) as AccountID
        ,CONCAT('5~', r.PRIM_ENC_CSN_ID) as VisitID
        ,null as VisitCaseID
        ,CASE WHEN r.MEDICATION_ID IS NOT NULL THEN CONCAT('5~','MED~', r.MEDICATION_ID) END as ItemID
        ,COALESCE(r.HCPCS_CODE, r.CPT_CODE) as ItemChargeCode
        ,r.REVENUE_CODE as ItemRevenueCode
        ,r.BILL_DESC AS ItemRevenueCodeDescription
        ,r.PROCEDURE_DESC AS ItemDescription
        ,'Medications' as ItemType
        ,'Medications' as ItemSubtype
        ,ISNULL(r.QUANTITY,0) AS VisitItemQuantity
        ,CASE WHEN ISNULL(r.QUANTITY,0) = 0 THEN ISNULL(r.COST,0) ELSE ISNULL(r.COST,0) / ISNULL(r.QUANTITY,1) END AS VisitItemUnitCost
        ,ISNULL(r.COST,0) as VisitItemExtendedCost
        ,CONVERT(DATE, r.SERVICE_DATE) as VisitItemUsedDate
    ...
    FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM], '
        ...
        LEFT JOIN CLARITY.ORGFILTER.HSP_ACCOUNT hsp ON hsp.HSP_ACCOUNT_ID = tx.HSP_ACCOUNT_ID 
        LEFT JOIN Clarity.ORGFILTER.CL_UB_REV_CODE rc ON rc.UB_REV_CODE_ID = tx.UB_REV_CODE_ID	
        LEFT JOIN Clarity.ORGFILTER.ORDER_MED ord ON ord.ORDER_MED_ID = tx.ORDER_ID
        WHERE tx.TX_TYPE_HA_C in (1)
