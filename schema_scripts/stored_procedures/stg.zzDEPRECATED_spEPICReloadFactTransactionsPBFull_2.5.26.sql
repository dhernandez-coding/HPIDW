...
CREATE PROCEDURE [stg].[spEPICReloadFactTransactionsPBFull] as
...
CREATE TABLE #Temp_EPIC_Dx (TransactionID varchar(100)							
...
SELECT
	CONCAT('5~',tx.TX_ID
		...
		,CASE WHEN tx.DETAIL_TYPE = 1 THEN 'Charge - New'
			   ...
			   WHEN tx.DETAIL_TYPE = 21 and tx.AMOUNT <= 0 THEN 'Credit Adjustment - Matched to Charge'
			   WHEN tx.DETAIL_TYPE = 21 and tx.AMOUNT > 0 THEN 'Debit Adjustment - Matched to Charge'
...
FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM],'
	...
	WHERE 1=1 
		AND tx.SERVICE_AREA_ID in (425,430,452)
...
FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM],
	...
	from (
		...
		,CASE WHEN t.DETAIL_TYPE IN (2,11) THEN t.TX_ID /* 1/27/26 - Chris Cross - Moved detail type 33 (Payment matched to debit adjustment) to section below which puts the source id as the Debit Adj transaction*/
			  WHEN t.DETAIL_TYPE IN (20,32,33) THEN t.MATCH_TRX_ID END as TransactionSourceID
		,CASE WHEN t.DETAIL_TYPE IN (2) THEN NULL
			  WHEN t.DETAIL_TYPE IN (11,32,33) THEN t.TX_ID
			  WHEN t.DETAIL_TYPE IN (20) THEN t.TX_ID END as TransactionParentSourceID
		...
		,CASE WHEN t.DETAIL_TYPE IN (2) THEN ''Payment - New''
			 ...
			 WHEN t.DETAIL_TYPE = 20 THEN ''Payment - Matched to Charge''
			 WHEN t.DETAIL_TYPE IN (33) THEN ''Payment - Refund/Reversal''
			 WHEN t.DETAIL_TYPE IN (32) THEN ''Payment - Distributed''
			 --WHEN t.DETAIL_TYPE = 20 and t.AMOUNT <= 0 THEN ''Payment - Matched to Charge''
			 --WHEN t.DETAIL_TYPE = 20 and t.AMOUNT > 0 THEN ''Payment - Refund/Reversal''
		...
		where 1=1
			AND t.SERV_AREA_ID IN (425,430,452) /*Added 452 on 1/27/26*/
			AND t.post_date >= ''1/1/2019'' 
			AND t.DETAIL_TYPE IN (2,11,20,32,33) 
...
/*--Removed on 1/27/26 by Chris Cross as no longer needed with previous matching to payments and charges
/*Load Debit adjustments into #Staging*/
	Print 'Step 1.3:  Loading Debit Adjustments into #Staging...'
	INSERT INTO #Staging
	([TransactionID]
      ,[TransactionDatasourceID]
      ,[TransactionSourceID]
