CREATE procedure [stg].[spMedhostNSReloadFactVisitProceduresFull] AS 

	DELETE FROM fact.VisitProcedures WHERE VisitProcedureDataSourceID = 2
	...
	SELECT 
	CONCAT('2~',v.PRNUMB,'~ICD10~',v.PRTYPE,'~',v.PRBSEQ#,'~',v.PRICD9) as  VisitProcedureID 
	,2 as VisitProcedureDataSourceID 
	,CONCAT(v.PRNUMB,'~',v.PRTYPE,'~',v.PRBSEQ#,'~',v.PRICD9) as VisitProcedureSourceID 
	,CONCAT('2~',v.PRNUMB) as VisitProcedureVisitID 
	,CONCAT('2~',v.PRNUMB) as VisitProcedureAccountID 
	...
	,CONCAT('2~',v.PRDOCN) as VisitProcedureProviderID 
  ...
  FROM OPENQUERY([hmsls],'
	...
	,prc.PRBSEQ#
	--prc.*
	from MHD32.HOSPF100.MRABPROC prc 
	...
	SELECT 
	CONCAT('2~',v.CPNUMB,'~CPT~',v.CPSEQ#,'~',v.CPSEQ#,'~',v.CPCPTC,'~',v.CPPSDT) as  VisitProcedureID 
	,2 as VisitProcedureDataSourceID 
	,CONCAT(v.CPNUMB,'~CPT~',v.CPSEQ#,'~',v.CPSEQ#,'~',v.CPCPTC,'~',v.CPPSDT) as VisitProcedureSourceID 
	,CONCAT('2~',v.CPNUMB) as VisitProcedureVisitID 
	,CONCAT('2~',v.CPNUMB) as VisitProcedureAccountID 
