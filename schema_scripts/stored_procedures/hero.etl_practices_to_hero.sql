CREATE PROCEDURE [hero].[etl_practices_to_hero]
AS
...
left join [HERo].Companiess company on prac.PracticeCompany = company.CompanyName
left join [hero].Specialtiess specialty on prac.PracticeSpecialty = specialty.SpecialtyName
--left join hpi_etl.dbo.Departmentss department on prac.Department = company.CompanyName
where prac.PracticeID not in (select PracticePracticeID from [hero].Practicess)
