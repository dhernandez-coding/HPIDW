...
CREATE PROCEDURE [stg].[spMedhostNSReloadFactAccountsFull]
as

DELETE FROM fact.Accounts WHERE AccountDataSourceID = 2
...
SELECT 
	  CONCAT('2~',v.PATNO) AS AccountID
      ,2 as AccountDataSourceID
      ,v.PATNO as AccountSourceID
      ,v.PATNO as AccountReferenceNumber
      ,CONCAT('2~',v.HSTNUM) as AccountPatientID
      ,CONCAT('2~',v.LOCATION) as AccountLocationID
	  ...
	  ,CONCAT('2~',CASE WHEN v.NWARFC1 = '0' THEN '999' ELSE v.NWARFC1 END) as AccountFinancialClassID
      ,CONCAT('2~',CASE WHEN v.AINS1 = '0' THEN '999' ELSE v.AINS1 END) as AccountPrimaryPayerID /*999 is self-pay*/
	  ,CONCAT('2~',CASE WHEN v.AINS1 = '0' THEN '999' ELSE v.AINS1 END,'~',CASE WHEN v.AINS1 = '0' THEN '1' ELSE v.APLN1 END) as AccountPrimaryPayerPlanID /*999 is self-pay*/
      ,CASE WHEN coalesce(v.NWPROCDR,v.PRDOCN, v.NWDOCNUM) IS NOT NULL THEN CONCAT('2~', coalesce(v.NWPROCDR,v.PRDOCN, v.NWDOCNUM)) END as AccountPrimaryProviderID
      ,CASE WHEN coalesce(v.NWATTPHY, NWPROCDR, v.NWDOCNUM) IS NOT NULL THEN CONCAT('2~', coalesce(v.NWATTPHY, NWPROCDR, v.NWDOCNUM)) END as AccountAdmittingProviderID
      ,CASE WHEN v.NWATTPHY IS NOT NULL THEN CONCAT('2~',v.NWATTPHY) END as AccountAttendingProviderID
      ,CASE WHEN v.NWREFDOC IS NOT NULL THEN CONCAT('2~',v.NWREFDOC) END as AccountReferringProviderID
