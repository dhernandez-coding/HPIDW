...
DROP TABLE IF EXISTS #TEMP_Collections 
		...
		SELECT
			...
			,pfs.FeeScheduleID as FeeScheduleID
		...
		WHERE 1=1
			...
			AND c.TransactionDateOfPosting >= '1/1/2023'--DATEADD(DAY,-365,GETDATE())
...
/*Load Temp Table with payment lag data by practice*/
	
	/* --Replaced by rpt.PBPaymentLag
	IF OBJECT_ID('tempdb..#TEMP_DaysToPay') IS NOT NULL DROP TABLE #TEMP_DaysToPay ---rpt.PBPaymentLag
