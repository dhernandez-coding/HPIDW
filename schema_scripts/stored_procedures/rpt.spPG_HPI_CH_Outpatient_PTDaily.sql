...
1/24/2024: Eric Silvestri: Updated CTEConfidentialPatients to include 430 Service area only 
...
CREATE PROCEDURE [rpt].[spPG_HPI_CH_Outpatient_PTDaily] @STARTDATE date, @ENDDATE date
AS
BEGIN
SET NOCOUNT ON;
...
/*Clear out TempPT table*/
IF OBJECT_ID(N'TempPTdb..##TempPT') IS NOT NULL
BEGIN
DROP TABLE ##TempPT
END
...
SET @filename
     = 'HPI_CH_OP_PT_Daily_' + CONVERT(NVARCHAR(10), @STARTDATE, 112) + '_to_' + CONVERT(NVARCHAR(10), @ENDDATE, 112)
...
select
	'OU0105' PressGaneySurgeyDesignatorEPT -- ER0101 reporttype/file hardcode
		,case
...
where 1=1
	...
	and eh.DEPARTMENT_ID IN (43005005025, 43005005026) /*HPI CHS OP PT QUAIL, HPI CHS OP PT SOUTH*/
...
'CAST(EORIndicator as varchar(1)) FROM ##TempPT'
