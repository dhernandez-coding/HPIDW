CREATE PROCEDURE [rpt].[spSelectBlueBooksPivot]
...
--DECLARE @CurrentPeriod int = 12 --MONTH(@AsOfDate)
--DECLARE @Practice varchar(10) = 'ACC' /*0 - All*/  --'DRW'
...
FROM (
	...
	SELECT [FiscalYear]
		  ...
		  ,p.PracticeCompany
		  ,p.PracticeIsActive 
		  ,CASE WHEN p.PracticeSameStoreDate <= DATEFROMPARTS(@CurrentYear,@CurrentPeriod,1) THEN 1
				ELSE 0 END as PracticeIsSameStore 
				/*10/30/25 - Chris Cross - Adjusted IsSameStore logic to be based on effective date*/
		  ,[ReportingProviderID]
