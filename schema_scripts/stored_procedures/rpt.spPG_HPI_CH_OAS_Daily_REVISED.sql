...
1/23/2024: Eric Silvestri: Included Department IDs 43004001001, 43005005001 for Med/Surg
...
12/30/2025: Chris Cross: Updated Admission Source from zcas.ADMIT_SOURCE_C to zcas.ABBR;  Revised version uses OPENQUERY
-- =============================================
*/

CREATE PROCEDURE [rpt].[spPG_HPI_CH_OAS_Daily_REVISED] @STARTDATE date, @ENDDATE date AS
...
--DECLARE @STARTDATE AS DATE = '7/1/2025'
--DECLARE @ENDDATE AS DATE = '12/29/2025'
...
SET @filename
     = 'HPI_CH_OAS_Daily_' + CONVERT(NVARCHAR(10), @STARTDATE, 112) + '_to_' + CONVERT(NVARCHAR(10), @ENDDATE, 112)


/*Clear out Temp Tables used in Query*/
DROP TABLE IF EXISTS #TempAccounts
DROP TABLE IF EXISTS #TempAccounts2
DROP TABLE IF EXISTS #TempAccounts3
DROP TABLE IF EXISTS #TempPivot
DROP TABLE IF EXISTS #cteProviders
DROP TABLE IF EXISTS #cteConfidentialPatients
DROP TABLE IF EXISTS ##TempCHOAS
DROP TABLE IF EXISTS #TempRef_Bill_Code
DROP TABLE IF EXISTS #TempRef_Bill_Code2

/*INSERT Filtered Records into #TempAccounts*/
	select
	sub.HSP_ACCOUNT_ID as HSP_ACCOUNT_ID
	,sub.PRIM_ENC_CSN_ID
	,sub.EXP_ADMISSION_TIME
	,sub.HOSP_ADMSN_TIME
	,sub.HOSP_DISCH_TIME
	,sub.EFFECTIVE_DATE_DT
	,sub.ADT_PATIENT_STAT_C
	,sub.APPT_STATUS_C
	,sub.department_id
	,sub.Survey_Opt_Out_YN
	,sub.DISCH_LOC_ID
	,sub.department_name
	,sub.TOT_ACCT_BAL
	INTO #TempAccounts
	...
	,cd.department_name
	,a.TOT_ACCT_BAL
	,CONVERT(DATE,a.CODING_DATETIME) AS CodingDate
	...
	where 1=1
		...
		and CONVERT(DATE,a.CODING_DATETIME) >= DATEADD(DAY,-365,GETDATE()) --between @STARTDATE and @ENDDATE
		AND a.CODING_STATUS_C IN (4)
		--AND CONVERT(DATE,a.CODING_DATETIME) = @ENDDATE
	') sub
	WHERE 1=1
		AND sub.CodingDate between @STARTDATE and @ENDDATE
...
SELECT sub.ref_bill_code as ref_bill_code 
INTO #TempRef_Bill_Code 
FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM],'
	SELECT DISTINCT ref_bill_code 
	FROM [CLARITY].[ORGFILTER].[V_CODING_ALL_DX_PX_LIST] 
...
--	,ISNULL(zcdd.name, '') DischargeDispName
...
--	,ISNULL(eh.ADT_PATIENT_STAT_C, '') DischargeStatus
	...
	,admp.ProviderSpecialty AdmittingProviderSpecialty
	,admp.prov_name AdmittingProviderName
	,ISNULL(a.attending_prov_id, '') AttendingProvID
	,ISNULL(attp.ProviderSpecialty, '') AttendingProviderSpecialty
	...
	,CASE WHEN pls.name = 'Alive' THEN 'N' 	
			WHEN pls.name = 'Deceased' THEN 'Y'  END AS PatientLivingStatus
...
WHERE 1=1
	...
	AND sub.DEPARTMENT_ID IN (43004001014, 43005005013, 43004001016, 43005005015, 43004001017, 43005005016, 43005005018, 43004001001, 43005005001) /*HPI CHN MAIN OR, HPI CHS MAIN OR, HPI CHN ENDOSCOPY, HPI CHS ENDOSCOPY, HPI CHN PAIN ,HPI CHS PAIN, HPI CHS OPHTHALMOLOGIC, HPI CHS MED/SURG HPI CHN MED/SURG*/
	--and (sub.TOT_ACCT_BAL is null or sub.TOT_ACCT_BAL <> 0)
...
'CAST(AdmittingProviderSpecialty as varchar(50)), '','',' +
'CAST(AdmittingProviderName as varchar(50)), '','',' +
'CAST(AttendingProvID as varchar(50)), '','',' +
'CAST(AttendingProviderSpecialty as varchar(50)), '','',' +
...
'CAST(EORIndicator as varchar(1)) FROM ##TempCHOAS'
