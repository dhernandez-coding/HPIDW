...
BEGIN
    ...
    SELECT
        concat('5~', r.or_case_id) VisitCaseID
        ,5 VisitCaseDatasourceID
        ,r.or_case_id VisitCaseSourceID
        ,case when r.or_link_csn is null then null else concat('5~', r.or_link_csn) end VisitCaseVisitID
        ,r.VisitCaseCSN
        ,concat('5~', r.loc_id) VisitCaseLocationID
        ,r.surgery_date VisitCaseServiceDate
        ,r.time_scheduled VisitCaseScheduledDatetime
        ,r.CASE_BEGIN_INSTANT as VisitCaseScheduleStartDateTime
        ,r.CASE_END_INSTANT as VisitCaseScheduleEndDateTime
        ,r.VisitCaseBeginDatetime
        ,r.VisitCaseEndDatetime
        ,r.VisitCaseRescheduledDate
        ,r.VisitCaseCancelledDate
        ,r.CancelledReasonName as VisitCaseCancelledReason
        ,r.VisitCaseIsRescheduled
        ,r.service_c VisitCaseServiceID
        ,r.or_id VisitCaseORID
        ,CASE 
            WHEN r.ROOM_ID is not null THEN CONCAT('5~',r.ROOM_ID)
            WHEN r.or_id IS NOT NULL THEN CONCAT('5~',r.or_id) 
         END as VisitCaseRoomID 
        ,r.VisitCaseScheduleStatus
        ,r.PatientClassName as VisitCasePatientClass
        ,r.ServiceName as VisitCaseService
        ,concat('5~', r.primary_physician_id) VisitCasePrimaryProviderID
        ,r.total_time_needed VisitCaseTotalTimeNeeded
        ,r.LogStatusName as VisitCaseLogStatus
        ,r.SCHEDULED_IN_OR_DTTM as VisitCaseScheduledORBeginDatetime 
        ,r.SCHEDULED_OUT_OR_DTTM as VisitCaseScheduledOREndDatetime 
        ,r.IN_PREOP_DTTM as VisitCasePreOpBeginDatetime 
        ,r.COMP_PREOP_DTTM as VisitCasePreOpEndDatetime
        ,r.IN_OR_DTTM as VisitCaseORBeginDatetime 
        ,r.PROCEDURE_START_DTTM as VisitCaseProcedureBeginDatetime 
        ,r.PROCEDURE_COMP_DTTM as VisitCaseProcedureEndDatetime 
        ,r.OUT_OR_DTTM  as VisitCaseOREndDatetime 
        ,r.IN_RECOVERY_DTTM as VisitCaseRecoveryBeginDatetime 
        ,r.OUT_RECOVERY_DTTM as VisitCaseRecoveryEndDatetime 
        ,r.IN_PHASEII_DTTM as VisitCasePhase2BeginDatetime 
        ,r.OUT_PHASEII_DTTM as VisitCasePhase2EndDatetime 
        ,r.MINUTES_SCHEDULED_IN_OR as VisitCaseMinutesScheduledInOR 
        ,r.MINUTES_IN_PREOP as VisitCaseMinutesInPreop 
        ,r.MINUTES_IN_OR as VisitCaseMinutesInOR 
        ,r.MINUTES_IN_RECOVERY as VisitCaseMinutesInRecovery 
        ,r.MINUTES_IN_PHASEII as VisitCaseMinutesInPhase2 
        ,r.SETUP_LENGTH as VisitCaseSetupMinutes
        ,r.CLEANUP_LENGTH as VisitCaseCleanupMinutes
        ,r.SCHED_ROOM_OUT_TO_IN as VisitCaseScheduledTurnoverDuration
        ,r.ROOM_OUT_TO_IN_ADJ as VisitCaseRoomTurnoverDuration
        ,r.PROC_COMP_TO_START_ADJ as VisitCaseProcedureTurnoverDuration
        ,r.MINUTES_LATE as VisitCaseMinutesLate 
        ,r.LATE_CASE_YN as VisitCaseLateStart
        ,r.MINUTES_OVERRUN as VisitCaseMinutesOverrun 
        ,r.CASE_LEN_MIN_DIFF as VisitCaseLenghtDifference 
        ,r.CASE_LEN_PCT_DIFF as VisitCaseLenghtDifferencePercent
        ,r.CASE_LEN_ACCURAT_YN as VisitCaseLengthAccuracy 
        ,CONVERT(varchar(300), r.PROC_NAME) as VisitCasePrimaryProcedure
        ,r.NotPerformedReasonName as VisitCaseNotPerformedReason 
        ,r.PROC_NOT_PERF_COM as VisitCaseNotPerformedComment
        ,r.HEIGHT as VisitCaseHeight 
        ,TRY_CAST(r.WEIGHT AS FLOAT) / 16 AS VisitCaseWeight 
        ,TRY_CAST(r.BMI AS FLOAT) AS VisitCaseBMI 
        ,CASE WHEN r.RESP_ANES_ID IS NULL THEN NULL ELSE CONCAT('5~',r.RESP_ANES_ID) END as VisitCaseAnesthesiaProviderID 
        ,r.VisitCaseAnesthesiaType 
        ,r.ASA_RATING_C as VisitCaseASARating 
        ,CASE 
            WHEN r.lb_primary_physician_id LIKE 'P%' 
                 THEN CAST(SUBSTRING(r.lb_primary_physician_id, 2, LEN(r.lb_primary_physician_id)) AS INT) 
            ELSE CAST(r.lb_primary_physician_id AS INT) 
         END AS VisitCasePhysician 
        ,r.PRIMARY_CIRCULATOR_ID as VisitCaseCirculatingNurse 
        ,r.PRIMARY_SURG_TECH_ID AS VisitCaseSurgTech 
        ,r.FIRST_CASE_IN_RNG_YN AS VisitCaseFirstCaseofDay 
        ,r.DISCH_DISP_C AS VisitCaseDischargeDisposition 
        ,r.TEMPERATURE_PACU_FIRST as VisitCasePatientTemperaturePACUArrival 
        ,r.VisitCaseAdmissionStatus 
        ,getdate() VisitCaseUpdatedDatetime
        ,r.PROC_DISPLAY_NAME as VisitCasePrimaryScheduledProcedure
    ...
    FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM], '
        SELECT
            ...
            orc.surgery_date,
            orc.time_scheduled,
            orc.CASE_BEGIN_INSTANT,
            orc.CASE_END_INSTANT,
            ...
            can.NAME AS CancelledReasonName,
            CASE WHEN csc.CASE_ID IS NOT NULL THEN ''Yes'' ELSE ''No'' END AS VisitCaseIsRescheduled,
            orc.service_c,
            orc.or_id,
            orl.ROOM_ID,
            CASE WHEN lb.IN_OR_DTTM IS NOT NULL THEN ''Completed'' ELSE sts.NAME END AS VisitCaseScheduleStatus,
            pc.NAME AS PatientClassName,
            svc.NAME AS ServiceName,
            orc.primary_physician_id,
            orc.total_time_needed,
            os.NAME AS LogStatusName,
            lb.SCHEDULED_IN_OR_DTTM,
            lb.SCHEDULED_OUT_OR_DTTM,
            lb.IN_PREOP_DTTM,
            lb.COMP_PREOP_DTTM,
            lb.IN_OR_DTTM,
            lb.PROCEDURE_START_DTTM,
            lb.PROCEDURE_COMP_DTTM,
            lb.OUT_OR_DTTM,
            lb.IN_RECOVERY_DTTM,
            lb.OUT_RECOVERY_DTTM,
            lb.IN_PHASEII_DTTM,
            lb.OUT_PHASEII_DTTM,
            lb.MINUTES_SCHEDULED_IN_OR,
            lb.MINUTES_IN_PREOP,
            lb.MINUTES_IN_OR,
            lb.MINUTES_IN_RECOVERY,
            lb.MINUTES_IN_PHASEII,
            ot.SETUP_LENGTH,
            ot.CLEANUP_LENGTH,
            rt.SCHED_ROOM_OUT_TO_IN,
            rt.ROOM_OUT_TO_IN_ADJ,
            rt.PROC_COMP_TO_START_ADJ,
            lv.MINUTES_LATE,
            lv.LATE_CASE_YN,
            lv.MINUTES_OVERRUN,
            lv.CASE_LEN_MIN_DIFF,
            lv.CASE_LEN_PCT_DIFF,
            lv.CASE_LEN_ACCURAT_YN,
            op.PROC_NAME,
            np.NAME AS NotPerformedReasonName,
            orl.PROC_NOT_PERF_COM,
            enc.HEIGHT,
            enc.WEIGHT,
            enc.BMI,
            lb.RESP_ANES_ID,
            anz.NAME AS VisitCaseAnesthesiaType,
            orl.ASA_RATING_C,
            lb.PRIMARY_PHYSICIAN_ID AS lb_primary_physician_id,
            lb.PRIMARY_CIRCULATOR_ID,
            lb.PRIMARY_SURG_TECH_ID,
            lv.FIRST_CASE_IN_RNG_YN,
            peh.DISCH_DISP_C,
            s.TEMPERATURE_PACU_FIRST,
        ...
        LEFT JOIN CLARITY.ORGFILTER.clarity_loc loc ON orc.loc_id = loc.loc_id
        LEFT JOIN CLARITY.ORGFILTER.PAT_OR_ADM_LINK ol ON orc.OR_CASE_ID = ol.OR_CASELOG_ID
        LEFT JOIN CLARITY.ORGFILTER.PAT_ENC enc ON ol.OR_LINK_CSN = enc.PAT_ENC_CSN_ID 
        LEFT JOIN CLARITY.ORGFILTER.ZC_OR_SCHED_STATUS sts ON orc.SCHED_STATUS_C = sts.SCHED_STATUS_C
        LEFT JOIN CLARITY.ORGFILTER.zc_or_status os ON orl.status_c = os.status_c
        LEFT JOIN CLARITY.ORGFILTER.ZC_OR_SERVICE svc ON orc.SERVICE_C = svc.SERVICE_C
        LEFT JOIN CLARITY.ORGFILTER.ZC_PAT_CLASS pc ON orc.PAT_CLASS_C = pc.ADT_PAT_CLASS_C
        LEFT JOIN CLARITY.ORGFILTER.ZC_OR_CANCEL_RSN can ON can.CANCEL_REASON_C = orc.CANCEL_REASON_C
        LEFT JOIN(
            SELECT CASE_ID
            FROM CLARITY.ORGFILTER.V_CASE_SCHEDULE_CHANGE
            WHERE RESCHED_TO_DATE IS NOT NULL
            GROUP BY CASE_ID
        ) csc ON csc.CASE_ID = orc.or_case_id
        LEFT JOIN CLARITY.ORGFILTER.ZC_PROC_NOT_PERF np ON np.PROC_NOT_PERF_C = orl.PROC_NOT_PERF_C
        LEFT JOIN CLARITY.ORGFILTER.OR_CASE_ALL_PROC cp ON cp.OR_CASE_ID = orc.OR_CASE_ID AND cp.LINE = 1
        LEFT JOIN CLARITY.ORGFILTER.OR_PROC op ON op.OR_PROC_ID = COALESCE(lb.PRIMARY_PROCEDURE_ID, cp.OR_PROC_ID)
        LEFT JOIN CLARITY.ORGFILTER.OR_LOG_VIRTUAL lv ON lv.LOG_ID = orc.OR_CASE_ID
        LEFT JOIN CLARITY.ORGFILTER.DM_SURGERY s ON s.LOG_ID = orc.OR_CASE_ID
        LEFT JOIN CLARITY.ORGFILTER.PAT_ENC_HSP peh ON ol.OR_LINK_CSN = peh.PAT_ENC_CSN_ID
        LEFT JOIN CLARITY.ORGFILTER.V_CASE_ON_TIME_START ot ON ot.case_id = orc.or_case_id
...
--	ALTER procedure [stg].[spEPICReloadFactVisitCasesFull] as
--	BEGIN
--    SET NOCOUNT ON;
