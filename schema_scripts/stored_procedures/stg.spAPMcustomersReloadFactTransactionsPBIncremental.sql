CREATE PROCEDURE [stg].[spAPMcustomersReloadFactTransactionsPBIncremental] as
...
DROP TABLE IF EXISTS #Temp_APM_Dx
CREATE TABLE #Temp_APM_Dx
(
    VisitID       varchar(100) NOT NULL,
    TransactionID int          NOT NULL,  -- Service_ID, no '12~' prefix
...
INSERT INTO #Temp_APM_Dx
	...
	FROM OPENQUERY([TIEVMDB03], '
	  ...
	  FROM Ntier_HPI_Customers.PM.Vouchers v 
		  INNER JOIN Ntier_HPI_Customers.PM.vwGenSvcDiagInfo d ON v.Voucher_ID = d.Voucher_ID
	  WHERE 1=1
		AND v.Billing_Date >= ''2023-01-01''
...
/*Load Charges into #Staging*/
	...
	([TransactionID]
	  ...
	  ,[TransactionPlaceOfService]
        ...
        CASE WHEN tx.Voucher_Number IS NOT NULL THEN CONCAT('12~',tx.Voucher_Number) END AS TransactionVisitID,
        CASE WHEN tx.Voucher_Number IS NOT NULL THEN CONCAT('12~',tx.Voucher_Number) END AS TransactionAccountID,
        ...
        CASE WHEN COALESCE(tx.Original_Carrier_ID,tx.Billing_Carrier_ID) IS NOT NULL
             THEN CONCAT('12~',COALESCE(tx.Original_Carrier_ID,tx.Billing_Carrier_ID))
             WHEN v.Is_Self_Pay = 1 THEN '12~SELF'
        END AS TransactionPayerID,
        CASE WHEN tx.Billing_Dr_ID = '18438' AND tx.Refer_Dr_ID IS NOT NULL
             THEN CONCAT('12~',tx.Refer_Dr_ID)
             WHEN tx.Billing_Dr_ID IS NOT NULL THEN CONCAT('12~',tx.Billing_Dr_ID )
        ...
        CONCAT('12~',v.Patient_ID) AS TransactionPatientID,
    ...
    FROM [TIEVMDB03].[Ntier_HPI_Customers].[PM].vwGenSvcInfo tx
		...
		LEFT JOIN [TIEVMDB03].[Ntier_HPI_Customers].[PM].POS_BILLING_CODES bc ON bc.Place_Of_Service_ID = v.Place_Of_Service_ID 
    ...
    GROUP BY
        CONCAT('12~',tx.Service_ID),
        CONVERT(varchar(50),tx.Service_ID),
        CONVERT(varchar(50),tx.Service_ID),
        CASE WHEN tx.Voucher_Number IS NOT NULL THEN CONCAT('12~',tx.Voucher_Number) END,
        CASE WHEN tx.Voucher_Number IS NOT NULL THEN CONCAT('12~',tx.Voucher_Number) END,
        CASE WHEN v.Voucher_Number IS NOT NULL THEN CONCAT('12~',v.Department_ID) END,
        CASE WHEN COALESCE(tx.Original_Carrier_ID,tx.Billing_Carrier_ID) IS NOT NULL
             THEN CONCAT('12~',COALESCE(tx.Original_Carrier_ID,tx.Billing_Carrier_ID))
             WHEN v.Is_Self_Pay = 1 THEN '12~SELF'
        END,
	...
	Print 'Step 1.2:  Loading Charges Voids into #Staging...'
	INSERT INTO #Staging
	([TransactionID]
      ,[TransactionDatasourceID]
      ,[TransactionSourceID]
	...
	FROM [TIEVMDB03].[Ntier_HPI_Customers].[PM].vwGenSvcInfo tx
		...
		left join [TIEVMDB03].[Ntier_HPI_Customers].[dbo].[vwGenPatApptInfo] a ON a.Appt_ID = v.Appointment_ID
		LEFT JOIN [HPIDW].[stg].[PBProcedureCodeCategories] pc on tx.Procedure_Code = pc.[ProcedureCode]
		--LEFT JOIN [TIEVMDB03].[Ntier_HPI_Customers].[PM].Practice_Information PRACTINFO ON PRACTINFO.Tenant_ID = tx.Tenant_ID
...
/*Load Payments and other adjustments into #Staging*/
	...
	SELECT --v.Voucher_Number,tx.update_status,
		...
		,convert(varchar(50),tx.Service_ID) AS TransactionParentSourceID
		,CASE WHEN svc.Voucher_Number is not null THEN CONCAT('12~',svc.Voucher_Number) END as TransactionVisitID
		,CASE WHEN svc.Voucher_Number is not null THEN CONCAT('12~',svc.Voucher_Number) END as TransactionAccountID
		,CASE WHEN v.Department_ID is not null THEN CONCAT('12~',v.Department_ID) END as TransactionDepartmentID
...
/*Check #Staging records and delete/reload into fact.TransactionsPB*/
	...
	IF (SELECT COUNT(1) FROM #Staging) > 1 
		BEGIN
		Print 'Step 2.1:  More than 1 record exists in #Staging.  Proceeding with DELETE...'
	...
	SELECT [TransactionID]
		  ...
		  ,[TransactionPlaceOfService]
	FROM #Staging
	--WHERE TransactionID = '5~422945012~82061025~42294501200441'
