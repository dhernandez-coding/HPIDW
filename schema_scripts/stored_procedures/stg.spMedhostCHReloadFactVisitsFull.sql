...
CREATE PROCEDURE [stg].[spMedhostCHReloadFactVisitsFull]
as

DELETE FROM fact.Visits WHERE VisitDataSourceID = 8

INSERT INTO fact.Visits
SELECT 
	  CONCAT('8~',v.PATNO) AS VisitID
      ,8 as VisitDataSourceID
      ,v.PATNO as VisitSourceID
      ,v.PATNO as VisitReferenceNumber
      ,CONCAT('8~',v.HSTNUM) as VisitPatientID
      ,CONCAT('8~',v.LOCATION) as VisitLocationID
      ,v.HSV_DESC as VisitDepartmentID
      ,v.ABRMBD as VisitRoom
      ,CASE WHEN v.DXICD9 is not null THEN CONCAT(v.DXICD9,' - ', v.DxDesc,' (ICD)') END as VisitPrimaryDiagnosis  -- MHD32.HOSPF110.MRABDIAG
      ...
      ,CONCAT('8~',v.AINS1) as VisitPrimaryPayerID
	  ,CONCAT('8~',v.AINS1,'~',v.APLN1) as VisitPrimaryPayerPlanID
      ,CASE WHEN coalesce(v.NWPROCDR,v.PRDOCN, v.NWDOCNUM) IS NOT NULL THEN CONCAT('8~', coalesce(v.NWPROCDR,v.PRDOCN, v.NWDOCNUM)) END as VisitPrimaryProviderID
      ,CASE WHEN coalesce(v.NWATTPHY, NWPROCDR, v.NWDOCNUM) IS NOT NULL THEN CONCAT('8~', coalesce(v.NWATTPHY, NWPROCDR, v.NWDOCNUM)) END as VisitAdmittingProviderID
      ,CASE WHEN v.NWATTPHY IS NOT NULL THEN CONCAT('8~',v.NWATTPHY) END as VisitAttendingProviderID
      ,CASE WHEN v.NWREFDOC IS NOT NULL THEN CONCAT('8~',v.NWREFDOC) END as VisitReferringProviderID
      ...
      ,/*CASE WHEN v.SVCLAS = 'M' THEN 'Inpatient'
	   ...
	   CASE WHEN v.ABIO = 'I' THEN 'Inpatient'
			...
			ELSE v.RECTYP END
			as VisitClass
      ,isnull(v.SVDESC, v.HSV_DESC) as VisitType
