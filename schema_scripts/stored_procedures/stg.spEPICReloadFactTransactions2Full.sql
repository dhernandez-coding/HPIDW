...
CREATE PROCEDURE [stg].[spEPICReloadFactTransactions2Full] 
AS
...
Change Control:
	...
	3. 09/19/2025 - Diego Hernandez - Adjustment Category 
...
BEGIN
    ...
    DECLARE @StagingTable TABLE (
        ...
        TransactionEncounterID VARCHAR(100),
        [TransactionDepartmentID] VARCHAR(100),
        ...
        [TransactionUnits] DECIMAL(10, 2),
        [TransactionAmount] MONEY,
        TransactionRVU DECIMAL(18, 2),
        ...
        [TransactionUpdatedDateTime] DATE,
		[TransactionAdjustmentCategory] VARCHAR(50)
    );
 ...
 AND tx.SERV_AREA_ID in (425,430);
  ...
  IF (SELECT COUNT(1) FROM @StagingTable) >= 10
    BEGIN
        BEGIN TRY
            BEGIN TRANSACTION;
            PRINT 'At least 10 HB transactions found. Deleting and reloading fact.Transactions2...';

            DELETE FROM fact.Transactions2 WHERE TransactionDataSourceID = 5 AND TransactionBillingType = 'HB';

            INSERT INTO fact.Transactions2 (
                TransactionDatasourceID, TransactionSourceID, TransactionParentSourceID, TransactionVisitID,
                TransactionAccountID, TransactionEncounterID, TransactionDepartmentID, TransactionPayerID,
                ...
                TransactionCPTCode, TransactionModifier1, TransactionModifier2, TransactionModifier3,
                TransactionModifier4, TransactionUnits, TransactionAmount, TransactionRVU,
                TransactionDateOfService, TransactionDateOfPosting, TransactionDateOfBilling, TransactionDateOfVoid,
                TransactionReportingPeriodID, TransactionStatus, TransactionIsActive, TransactionUpdatedDateTime,TransactionAdjustmentCategory
            )
    ...
    ELSE
    BEGIN
        PRINT 'Less than 10 transactions found. Skipping delete and reload.';
    END
END;

 

 /* --Removed on 9/4/24 by Chris Cross
