CREATE PROCEDURE [stg].[spMedhostCHReloadFactTransactionsIncremental]
as

BEGIN
/*Deletes last 10 days of posted transactions and reloads just those days*/
DELETE FROM fact.Transactions WHERE TransactionDataSourceID = 8 AND TransactionDateOfPosting >= convert(date,DATEADD(DAY,-10,GETDATE()))
