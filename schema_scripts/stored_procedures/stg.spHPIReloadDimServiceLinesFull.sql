...
-- =============================================
CREATE PROCEDURE [stg].[spHPIReloadDimServiceLinesFull]
AS
BEGIN
SET NOCOUNT ON;


 DELETE FROM dim.ServiceLines WHERE ServiceLineDataSourceID = 0

 INSERT INTO dim.ServiceLines
 ([ServiceLineID],
	[ServiceLineDataSourceID],
	[ServiceLineSourceID],
	[ServiceLineName],
	ServiceLineGroupID,
	[ServiceLineIsActive],
	[ServiceLineUpdatedDateTime]
	)

 SELECT
	concat('0~',s.ServiceLineID) as ServiceLineID
	,0 as ServiceLineDatasourceID
	,CONVERT(varchar(100),s.ServiceLineID) as ServiceLineSourceID
	,s.ServiceLineName 
	,CASE WHEN s.ServiceLineGroupID is not null THEN concat('0~',s.ServiceLineGroupID) END as ServiceLineGroupID
    ,[ServiceLineIsActive]
	,GETDATE() AS [ServiceLineUpdatedDateTime]
 --select * 
 FROM [HPIApp].dbo.ServiceLines s
