...
FROM OPENQUERY([CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM],
	...
	from [Clarity].[ORGFILTER].CLARITY_TDL_TRAN t
		...
		LEFT JOIN [Clarity].[ORGFILTER].CLARITY_DEP dep ON dep.DEPARTMENT_ID = t.DEPT_ID
		LEFT JOIN [Clarity].[ORGFILTER].CLARITY_EAP eap1 ON eap1.PROC_ID = t.PROC_ID
		LEFT JOIN [Clarity].[ORGFILTER].CLARITY_EAP eap2 ON eap2.PROC_ID = t.MATCH_PROC_ID
		...
		LEFT JOIN [Clarity].[ORGFILTER].CLARITY_POS pos ON pos.POS_ID = t.POS_ID
	where 1=1
