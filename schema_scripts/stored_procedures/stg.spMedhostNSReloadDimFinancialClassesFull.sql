...
-- =============================================
CREATE PROCEDURE [stg].[spMedhostNSReloadDimFinancialClassesFull]
AS
BEGIN
SET NOCOUNT ON;

delete from dim.FinancialClasses where FinancialClassDataSourceID = 2
...
--select * from openquery([hmsls],'select FNWCLS, MAX(FCLSN) AS FinancialClass from MHD32.EISF110.EISCLSL1 GROUP BY FNWCLS ')
	...
	select 
		CONCAT('2~',FNWCLS) as FinancialClassID
		,2 as FinancialClassDataSourceID
	 ...
	 from openquery([hmsls],'select FNWCLS, MAX(FCLSN) AS FinancialClass from MHD32.EISF100.EISCLSL1 GROUP BY FNWCLS ') as F
