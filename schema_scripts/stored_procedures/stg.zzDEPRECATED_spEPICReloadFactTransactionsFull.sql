...
CREATE PROCEDURE [stg].[spEPICReloadFactTransactionsFull] as

BEGIN
--TRUNCATE TABLE fact.Transactions
DELETE FROM fact.Transactions WHERE TransactionDataSourceID = 5
...
--	,NULL AS TransactionDateOfVoid
 ...
 --select EAP.* 
 FROM [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].HSP_TRANSACTIONS tx
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].HSP_ACCOUNT a ON a.HSP_ACCOUNT_ID = tx.HSP_ACCOUNT_ID
	--LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].ZC_TX_TYPE_HA typ ON typ.TX_TYPE_HA_C = tx.TX_TYPE_HA_C
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].CLARITY_EAP eap ON eap.PROC_ID = tx.PROC_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].CL_UB_REV_CODE rc ON rc.UB_REV_CODE_ID = tx.UB_REV_CODE_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].HSP_TX_STATUS sts ON sts.TX_ID = tx.TX_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].CLARITY_EPP epp ON epp.BENEFIT_PLAN_ID = tx.PRIMARY_PLAN_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].ORDER_PROC op ON op.ORDER_PROC_ID = tx.ORDER_ID
 WHERE 1=1 
...
--	,tx.VOID_DATE as TransactionDateOfVoid
	...
	,1 as TransactionIsActive
	,GETDATE() as TransactionUpdatedDatetime
FROM [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].ARPB_TRANSACTIONS tx 
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].CLARITY_TDL_TRAN tdl ON tx.TX_ID = tdl.TX_ID 
	...
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[ZC_TRAN_TYPE] t ON t.TRAN_TYPE = tx.TX_TYPE_C
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[ARPB_TX_MODERATE] tm ON tm.TX_ID = tx.TX_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[HSP_ACCOUNT] a ON a.HSP_ACCOUNT_ID = tm.HOSP_ACCT_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[CLARITY_EAP] eap1 ON eap1.PROC_ID = tx.PROC_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[CLARITY_EAP] eap2 ON eap2.PROC_ID = tdl.PROC_ID
	LEFT JOIN [DBSQ_CLARITY.CORP.INTEGRIS-HEALTH.COM].[Clarity].[ORGFILTER].[CL_UB_REV_CODE] rc ON rc.UB_REV_CODE_ID = eap1.UB_REV_CODE_ID
WHERE 1=1 
