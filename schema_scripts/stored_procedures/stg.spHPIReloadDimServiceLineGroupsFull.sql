...
-- =============================================
CREATE PROCEDURE [stg].[spHPIReloadDimServiceLineGroupsFull]
AS
BEGIN
SET NOCOUNT ON;


 DELETE FROM dim.ServiceLineGroups WHERE ServiceLineGroupDataSourceID = 0

 INSERT INTO dim.ServiceLineGroups
 ([ServiceLineGroupID],
	[ServiceLineGroupDataSourceID],
	[ServiceLineGroupSourceID],
	[ServiceLineGroupName],
	[ServiceLineGroupIsActive],
	[ServiceLineGroupUpdatedDateTime]
	)

 SELECT
	concat('0~',s.ServiceLineGroupID) as ServiceLineGroupID
	,0 as ServiceLineGroupDatasourceID
	,CONVERT(varchar(100),s.ServiceLineGroupID) as ServiceLineGroupSourceID
	,s.ServiceLineGroupName 
    ,[ServiceLineGroupIsActive]
	,GETDATE() AS [ServiceLineGroupUpdatedDateTime]
 --select * 
 FROM [HPIApp].dbo.ServiceLineGroups s
