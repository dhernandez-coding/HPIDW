...
-- Description:	Extracts, Transforms and Loads Provider Data from Medhost Northwest Surgical Hospital Source System into a dim Table
-- Change Control
--	1. 03/14/2023 - Chris Cross - Initial build of procedure
-- =============================================
CREATE PROCEDURE [stg].[spMedhostNSReloadDimPayerCategoriesFull]
AS
BEGIN
SET NOCOUNT ON;

delete from dim.PayerCategories where PayerCategoryDataSourceID = 2
...
(
	...
	select
		CONCAT('2~','SP') AS PayerCategoryID
		,2 as PayerCategoryDatasourceID
	...
	select 
		CONCAT('2~',ITYPE) as PayerCategoryID
		,2 as PayerCategoryDataSourceID
