...
CREATE PROCEDURE [stg].[spMedhostNSReloadFactVisitItemsFull]
as

DELETE FROM fact.VisitItems WHERE VisitItemDataSourceID = 2
...
SELECT
	--top 100
	--CONCAT('2~',cc.CostTxnID) [VisitItemID]
	2 as [VisitItemDatasourceID]
	,CONCAT('2~', cc.ADTCaseNum) [AccountID]
	,CONCAT('2~', cc.ADTCaseNum) [VisitID]
	,CONCAT('2~', cc.ORCase) [VisitCaseID]
	,CONCAT('2~',coalesce(cc.itemid, cc.implantid, cc.ni_implant_id)) as [ItemID]
...
left join [CORHPIVMAP14].[DATA_PIMS_OK_OklahomaCity_NorthwestSurgicalHospital].dbo.pims_materials_item_type it on mi.itemtype = it.ItemType
left join
--select * from
	openquery(hmsls, 
		'	select
...
INSERT INTO fact.VisitItems
select 
	--CONCAT('2~CHG~',c.PATNO,'~',c.BATCH,'~',c.SEQ#,'~',c.REF#,'~',c.SVCCD) [VisitItemID]
	2 as [VisitItemDatasourceID]
	,CONCAT('2~', c.PATNO) [AccountID]
	,CONCAT('2~', c.PATNO) [VisitID]
	,NULL [VisitCaseID]
	,CONCAT('2~',c.ChargeCode) as [ItemID]
