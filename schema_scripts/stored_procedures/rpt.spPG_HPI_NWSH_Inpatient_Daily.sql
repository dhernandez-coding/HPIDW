...
CREATE PROCEDURE [rpt].[spPG_HPI_NWSH_Inpatient_Daily] @STARTDATE date, @ENDDATE date
AS
...
---        Service Area = 430
---        Location = HPI NORTHWEST SURGICAL HOSPITAL
...
IF OBJECT_ID(N'tempdb..##TempNWIP') IS NOT NULL
DROP TABLE ##TempNWIP
...
set @filename
     = 'HPI_NWSH_IP_Daily_' + CONVERT(NVARCHAR(10), @STARTDATE, 112) + '_to_' + CONVERT(NVARCHAR(10), @ENDDATE, 112)
...
--left join [CLARITYRDBMS.CORP.INTEGRIS-HEALTH.COM].[CLARITY].[ORGFILTER].[ZC_DISP_ENC_TYPE] et ON e.ENC_TYPE_C = et.DISP_ENC_TYPE_C
		...
		INNER join [map].[SurveyLanguageCodes] S ON zcl.language_c = s.language_c
...
where
	...
	and eh.department_id = 43006001001
...
'CAST(EORIndicator as varchar(1)) FROM ##TempNWIP'
