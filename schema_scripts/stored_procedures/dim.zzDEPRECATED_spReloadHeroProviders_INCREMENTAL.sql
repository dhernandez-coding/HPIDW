...
-- Edit date:   
-- Description:	INCREMENTAL reload for dim.Providers--_Hero from HPI App
-- ============================================= 
*/
BEGIN
...
/*-----DELETE/DEACTIVATE old records----*/
	...
	DELETE FROM dim.Providers--_Hero --WHERE ProviderDataSourceID = 0 AND <Insert Incremental Date Column> between 2026-07-21 AND 2026-08-20
...
/*----------Commented out for INCREMENTAL reload based on modified date range----------
	...
	FROM dim.Providers--_Hero target
...
/*-----INSERT new records-----*/
	...
	INSERT INTO dim.Providers--_Hero
	(ProviderID
	,ProviderDataSourceID
	,ProviderSourceID
	,ProviderAbbreviation
	...
	--	LEFT JOIN dim.Providers--_Hero target ON target.ProviderID = source.ProviderID
	WHERE 1=1
